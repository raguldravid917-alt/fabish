...
import { Printer, X, Download, Building2, MapPin, Phone, Mail, Globe } from 'lucide-react';
    ...
    <>
      ...
      <div className="fixed inset-0 z-[9999] bg-black/70 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto">
        ...
        >
            ...
            <table className="invoice-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              ...
              <tbody>
                {(order.orderItems || []).map((item, idx) => {
                  ...
                  // Back-calculate taxable (pre-tax) from inclusive price
                  ...
                  return (
                    <tr key={idx} style={{ borderBottom: '1px solid #f3f4f6', background: idx % 2 === 1 ? '#fafafa' : 'white' }}>
                      ...
                      <td style={{ padding: '12px' }}>
                        ...
                        {item.sku && <div style={{ fontSize: '10px', color: '#9ca3af', marginTop: '2px' }}>SKU: {item.sku}</div>}
                      </td>
                      <td style={{ padding: '12px', textAlign: 'center', color: '#6b7280', fontFamily: 'monospace' }}>
                        {item.hsnCode || '3304'}
                      </td>
                      <td style={{ padding: '12px', textAlign: 'center', fontWeight: '700', color: '#111827' }}>
                        {item.qty}
                      </td>
                      <td style={{ padding: '12px', textAlign: 'right', fontFamily: 'monospace', color: '#374151' }}>
                        {formatInr(item.price)}
                      </td>
                      <td style={{ padding: '12px', textAlign: 'right', color: '#6b7280' }}>
                        {itemGstRate}%
                      </td>
                      <td style={{ padding: '12px', textAlign: 'right', fontWeight: '700', color: '#111827', fontFamily: 'monospace' }}>
                        {formatInr(lineTotal)}
                      </td>
                    </tr>
                  );
          ...
          {/* ═══ TAX SUMMARY ═══════════════════════════════════════════════════ */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0', borderTop: '2px solid #2f3e10' }}>
          ...
          <div style={{ borderTop: '1px solid #e5e7eb', padding: '20px 32px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '24px', fontSize: '10px', color: '#6b7280' }}>
              <div>
                ...
                <p style={{ lineHeight: '1.5', margin: 0 }}>
                  Returns accepted within 7 days of delivery for unopened, unused items in original packaging. Contact support@fabish.in for return requests.
                </p>
              </div>
              <div>
