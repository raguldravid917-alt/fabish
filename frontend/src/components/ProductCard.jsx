...
const ProductCard = ({ product, onQuickView }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 sm:top-3 sm:right-3 z-20 flex flex-col gap-1.5 opacity-100 transition-all duration-300">
        {/* Wishlist Button */}
        <button
          onClick={() => toggleWishlist(product)}
          className={`w-7 h-7 sm:w-9 sm:h-9 rounded-full flex items-center justify-center shadow-md backdrop-blur-md transition-all duration-300 border border-white/50 cursor-pointer hover:scale-110 ${
            isWishlisted 
        ...
        >
          <Heart className="w-3.5 h-3.5 sm:w-4 sm:h-4" strokeWidth={1.8} fill={isWishlisted ? 'currentColor' : 'none'} />
        </button>
        ...
        <button
          ...
          className="w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-white/90 backdrop-blur-md text-[#1c2415] hover:bg-[#3a4d23] hover:text-white flex items-center justify-center shadow-md transition-all duration-300 border border-white/50 cursor-pointer hover:scale-110"
          title="Quick View"
          aria-label="Quick View"
        >
          <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4" strokeWidth={1.8} />
        </button>
      </div>

      {/* Image Container */}
      <div className="relative overflow-hidden aspect-[4/5] bg-[#f7f6f0] rounded-xl mb-2 sm:mb-3 flex items-center justify-center">
        <Link to={`/products/${product.slug}`} className="block w-full h-full">
          ...
          {hoverImage && hoverImage !== mainImage && (
            <img
              src={hoverImage}
              alt={product.title}
              className="absolute top-0 left-0 w-full h-full object-contain transition-all duration-700 ease-out opacity-0 group-hover:opacity-100 group-hover:scale-105"
            />
          )}
        </Link>
      </div>
      ...
      <div className="flex flex-col flex-grow text-center justify-between p-1">
        <div>
          <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-[#729855] block mb-1 font-heading line-clamp-1">
            {typeof product.category === 'object' ? product.category?.name : (product.category || 'Organic Skincare')}
          </span>
          <h3 className="font-heading font-medium text-xs sm:text-sm leading-snug text-[#1c2415] hover:text-[#729855] mb-2 line-clamp-2 min-h-[32px] sm:min-h-[36px] transition-colors break-words">
        ...
        <div className="mt-auto">
          <div className="flex items-center justify-center gap-1 mb-1.5 flex-wrap">
            ...
            <span className="text-[9.5px] sm:text-[10px] text-gray-500 font-body ml-0.5">(4.9)</span>
          </div>

          <div className="flex items-center justify-center gap-1.5 mb-2.5 font-body flex-wrap min-w-0">
            <span className="text-xs sm:text-sm font-bold text-[#1c2415]">
