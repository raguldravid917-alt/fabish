...
const PrivacyPolicy = () => {
  useDocumentTitle('Privacy Policy - Fabish');
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        const res = await cmsService.getPage('privacy-policy');
        if (res.success && res.data) {
          setPageData(res.data);
        } else {
          setError(res.message || 'Failed to load privacy policy data');
        }
      } catch (err) {
      ...
      >
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="text-[12px] font-bold text-[#666] tracking-[0.2em] uppercase">
            ...
            <span className="text-[#8b5a2b]">Privacy Policy</span>
          </p>
        </div>
      </div>
